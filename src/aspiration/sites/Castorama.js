...
(function (Castorama) {
	...
	Castorama.Castorama_GetArticles = function (html, obj) {
		var $ = cheerio.load(html);
		...
		list.each(function() {
			...
			if (hrefProduit && hrefProduit.length > 0) {
				...
				engine.AddRequest(hrefProduit, {}, {}, Castorama.Castorama_GetDetailsArticle, newObj);
			}
		});
	};

	Castorama.Castorama_Pagination = function (html, obj) {
		var $ = cheerio.load(html);
		...
		if (pagination.length) {
			...
			engine.AddRequest(url, {}, {}, Castorama.Castorama_Pagination, obj);
		}
	};

	Castorama.Castorama_SubNavigation = function (html, obj) {
		var $ = cheerio.load(html);
	...
	Castorama.Castorama_Navigation = function (html, obj) {
		var $ = cheerio.load(html);
	...
	Castorama.Castorama_PatchMagasin2 = function (html, obj) {
	...
	Castorama.Castorama_TraiterMagasin = function (html, obj) {
		var $ = cheerio.load(html);
	  ...
	  /*if (obj.MagasinId == "4234" || obj.MagasinId == "4252") {
		...
		//if (url && engine.shouldBeDone(obj.MagasinId)) {
			...
			engine.BindRequest(url, {}, {}, Castorama.Castorama_PatchMagasin2, clone);
		}
	...
	Castorama.Castorama_Patch0 = function (html, obj) {
		...
		if (engine.shouldBeDone(0)){
		  ...
		  engine.AddRequest("/store/", {}, {}, Castorama.Castorama_Navigation, clone);
		}
	...
	Castorama.debug = function (param) {
		var obj = {};
		obj.hostname = "www.castorama.fr";
		obj.Enseigne = name;
		obj.tree = [];
