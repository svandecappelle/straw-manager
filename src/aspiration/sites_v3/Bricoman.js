...
Bricoman.prototype.Bricoman_Fiche = function(html, obj) {
  console.log("Bricoman_Fiche".green.bold);
	...
	if ($('.std .top-content').text().trim() == "Produit non disponible") {
		...
		if (this.eventEmitter){
			this.eventEmitter.emit('error', output);
		}
	}
	/* ------------------------------------------------------------------------ */
	...
	if (this.eventEmitter){
    this.eventEmitter.emit('done', output);
	}
...
Bricoman.prototype.HomeMag = function(html, obj) {
	var $ = cheerio.load(html);
	...
	engine.AddRequest(newObj.lookup, {}, {}, this.Bricoman_Fiche, newObj);

};

Bricoman.prototype.Patch_magIN = function(html, obj) {
	var $ = cheerio.load(html);
	logger.info("Rentré dans Bricoman_MagasinList");
	logger.debug('***********[AFTER POST'+obj.MagasinId+']***********')
	var Req = engine.clone(obj);
	engine.AddRequest('/', {}, {}, this.HomeMag, Req);
};

Bricoman.prototype.Bricoman_MagasinList = function(html, obj) {
  var that = this;
	var $ = cheerio.load(html);
	var magArrayObj = {};
  ...
  var urlATraiter = [];
	...
	$("[id='shop_chooser'] option").each(function(idx) {
		...
		if(MagasinId == obj.MagId){
			...
			logger.info("Entrer dans le magasin " + MagasinId)
			var ReqObject = engine.clone(obj);
			ReqObject.tree = [];
      ...
      urlATraiter.push(ReqObject);

		}
	});

  for (var i = 0; i < urlATraiter.length; i++) {
    engine.BindRequest(urlATraiter[i].tmpurl, {}, {}, function(){
      console.log(that);
      this.Patch_magIN
    }, urlATraiter[i]);
  }
...
Bricoman.prototype.setEventEmitter = function (eventEmitter) {
  Bricoman.eventEmitter = eventEmitter;
};

Bricoman.prototype.update = function(param) {
	...
	engine.BindRequest("https://www.bricoman.fr/nos-magasins.html", {}, {}, this.Bricoman_MagasinList, obj);
};

Bricoman.prototype.debug = function(param) {
	engine._init(param.Enseigne);
